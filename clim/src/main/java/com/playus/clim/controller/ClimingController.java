...
public class ClimingController {
	...
	public int subscribingForCliming(@DestinationVariable int climNo) {
		...
		//List<ClimingList> climingList = climingListService.getClimRealtimeFeature(climNo);
		return climNo;
	...
	/*
	 * 방장들로부터 현재 영화 시점, 영화 번호를 받아 처리하여
	 * 시청자들에게 스틸컷, 장르, 포스터를 브로드캐스팅	 */
	@MessageMapping("/climing/respond/feature/currentTime/{currentTime}/movie/{movieNo}")
	@SendTo("/topic/climing/set/feature")
	public List<Movie> retrunClimFeature(@DestinationVariable int currentTime,
										@DestinationVariable int movieNo){
		//mapper미작성 : 질문할 것
